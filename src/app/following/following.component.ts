...
export class FollowingComponent implements OnInit {
	...
	constructor(private imagesService: ImagesService,
	    ...
	    // console.log("[*] following this.userIdentification " + this.userIdentification);
	    // if(this.userIdentification == null || this.userIdentification == ""){
	    // 	this.flagView = false;
	    // 	console.log("**** Bandera "+this.flagView);
	    // }else{
	    // 	this.flagView = true;
	    // 	console.log("**** Bandera "+this.flagView);
	    // }
		...
		const newGridLayout = new GridLayout();			
	  	...
	  	this.getAllFollowingUsers(this.userIdentification).then(allUsersFollowing => {	  		
		  	...
		  	// 		if(this.flagView==true){
					...
					// }
					var labelfollowbutton = "unfollow";
		  			var contadorRow = index;
		  			var elementRecordUseridId = elementRecord.userid;
			  	...
			  	// 		if(this.flagView==true){
						...
						button.on("tap", ( data ) => {
		  				...
		  				// 	}else{
								this.onClickFollowUser(elementRecord, tagButton);		  						
		  					// }
  		...
  		this.getAllFollowingShops(this.userIdentification).then(allShopFollowing => {
			...
			allShopFollowing.forEach( async (elementRecord, index) => {
				...
				this.getImageFilter(elementRecord.markerid.images[0]).then(dataImage=> {
					...
					button.on("tap", ( data ) => {
						let tagButton = <Button>data.object;
						this.onClickFollowShop(elementRecord, tagButton);
	...
	// public verifyFollow(idFollowerUser):string {

	// 	console.log("*************** " + idFollowerUser);

	// 	var labelIsfollow = "";

	//   	this.isFollower(this.userIdentification, idFollowerUser).then(dataIsFollow => {

	//         this.responseUsersFollow = dataIsFollow;

	//         if(this.responseUsersFollow.length>0){
	//             if(this.responseUsersFollow[0].status == false){
	//               // this.labelfollowbutton = localize("follow");
	//               labelIsfollow = "follow";

	//             }else if(this.responseUsersFollow[0].status == true){
	//               // this.labelfollowbutton = localize("unfollow");
	//               labelIsfollow = "unfollow";
	//             }            	
	//         }else{
	//         	labelIsfollow = "follow";
	//         	// this.labelfollowbutton = localize("follow");
	...
	//         console.log("return principal " + labelIsfollow);

	//         return labelIsfollow;
	
	//     });

	//     console.log("return secundario " + labelIsfollow);
	//     return labelIsfollow;
	// }  	

	onClickFollow(elementRecord, tagButton){
	 ...
	 onClickFollowShop(elementRecord, tagButton){
		    ...
		    //   objectUpdateFollowers.userid = this.userIdentification;
		    //   objectUpdateFollowers.markerid._id = this.profile_id_selected._id;
		    //   objectUpdateFollowers.status = true;
    ...
    public goBackProfile() {
        ...
        if( jsonuseraux == null)
            ...
            this.getDealsSubscribe(this.mainUserSaveIdentification).then(dealsResponse => {
